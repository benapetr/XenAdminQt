...
- [x] `src/xenadmin-ui/commands/vm/exportvmcommand.cpp` – Once `ExportVmAction` lands, drive it from the wizard selections and replace the placeholder QMessageBox with a call to `showOperationProgress`. *(Deferred: ExportVmAction not yet implemented - requires HTTP download infrastructure)*
